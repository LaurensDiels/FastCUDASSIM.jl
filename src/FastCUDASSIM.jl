...
* In these conversions we will (if necessary) first expand the channels dimensions, followed
...
* When we internally allocate an output (D)SSIM or gradient batch (either because an
  ...
  we will convert this back to a similar format as the first input image (batch). For
  example, if no batch axis appears here, then we will return a scalar (D)SSIM value instead
  of a `CuVector` of `length` `1`. There might be some promotions, though. For example,
  `ssim_gradient(imgs1, imgs2)` for `imgs1` a `CuArray{RGB{N0f8}, 3}` of `size`
