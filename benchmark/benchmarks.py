...
print("Single full HD image pair SSIM:")
h, w, b = 1080, 1920, 1
for c in (1, 3):
    print(f"  {c} channel{'' if c == 1 else 's'}:")
    ...
    time_ssim(pytorch_msssim.ssim, imgs1, imgs2, 3_000 // c)
...
print("DSSIMs and gradients batch (32 x c x 256 x 256):")
h, w, b = 256, 256, 32
for c in (1, 3):
    print(f"  {c} channel{'' if c == 1 else 's'}:")
    imgs1 = torch.empty(b, c, h, w, dtype=torch.float32, device="cuda")
