...
println("Single full HD image pair SSIM:")
h, w, b = 1080, 1920, 1
for (c, COL) in ((1, Gray), (3, RGB))
    println("  $c channel$(c == 1 ? "" : "s"):")

    println("    ImageQualityIndexes (CPU):")
    img1_iqi = rand(COL{Float32}, h, w)
    img2_iqi = rand(COL{Float32}, h, w)
    @benchmark(
...
println("DSSIMs and gradients batch (c x 256 x 256 x 32):")
h, w, b = 256, 256, 32
for c in (1, 3)
    println("  $c channel$(c == 1 ? "" : "s"):")
    ...
    N_dssims_dQMP = CuArray{Float32}(undef, h, 3, c, w, b)
    @benchmark(
        CUDA.@sync begin
            dssim_with_gradient!(
                $dssims, $dL_dimgs1,
